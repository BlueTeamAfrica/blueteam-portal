...
export default function ClientTicketDetailPage() {
  ...
  useEffect(() => {
    ...
    if (!user || !tenantId || !ticketId || !clientId) {
      setLoading(false);
      return;
    }
    ...
    load();
  }, [user, tenant?.id, ticketId, clientId]);
  ...
  async function handleSendReply(e: React.FormEvent) {
    e.preventDefault();
    const tenantId = tenant?.id;
    if (!user || !tenantId || !ticketId || !clientId) return;
  ...
  if (notFound || !ticket) {
    return (
      <div className="max-w-full min-w-0 space-y-4">
        <Link href="/client/support" className="text-[#4F46E5] hover:underline text-sm">
          ← Back to support
        </Link>
  ...
  return (
    <div className="max-w-full min-w-0 space-y-6 md:space-y-8">
      <Link href="/client/support" className="text-[#4F46E5] hover:underline text-sm inline-block">
        ← Back to support
      </Link>
      ...
      <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5">
        ...
        {replies.length === 0 ? (
          <div className="py-8 text-center rounded-lg border border-dashed border-slate-200 bg-slate-50/50">
            ...
            <p className="text-slate-400 text-xs mt-1">Send the first message to start the thread.</p>
          </div>
        ) : (
